...
app.post("/api/events", (req, res) => {
  const data = loadData();
  const { title, date, time, venue, capacity, duration, category, creatorRegNumber, volunteers } = req.body;
  ...
  // Prepare volunteers list if provided
  let volunteerObjects = [];
  // Volunteers can be provided as array of { regNumber, role } objects
  const volunteerInputs = Array.isArray(volunteers) ? volunteers : [];
  if (volunteerInputs.length > 0) {
    const roleSet = new Set();
    for (const v of volunteerInputs) {
      const reg = String(v && v.regNumber || '');
      const role = String(v && v.role || '').trim();
      if (!reg || !role) { return res.status(400).json({ ok: false, error: "Each volunteer must include regNumber and role." }); }
      const u = data.users.find(u => u.regNumber === reg);
      if (!u) { return res.status(400).json({ ok: false, error: `Volunteer not found: ${reg}` }); }
      if (reg === creatorRegNumber) { return res.status(400).json({ ok: false, error: "Organizer cannot be a volunteer." }); }
      if (roleSet.has(role)) { return res.status(400).json({ ok: false, error: `Duplicate role not allowed: ${role}` }); }
      roleSet.add(role);
    }
  }

  const newEvent = { id: Date.now(), title, date, time, venue, category, capacity: Number(capacity), duration: Number(duration), taken: 0, bookings: [], waitlist: [], volunteers: [], creatorRegNumber, liveNotificationSent: false };
  // assign volunteerIds and notify
  if (volunteerInputs.length > 0) {
    volunteerObjects = volunteerInputs.map((v, idx) => {
      const u = data.users.find(us => us.regNumber === v.regNumber);
      const volId = `V${String(idx+1).padStart(2,'0')}`;
      return { regNumber: v.regNumber, name: u ? u.name : v.regNumber, volunteerId: volId, role: String(v.role).trim() };
    });
    newEvent.volunteers = volunteerObjects;
  }
  ...
  if (volunteerObjects.length > 0) {
    volunteerObjects.forEach(v => {
      if (!data.notifications[v.regNumber]) data.notifications[v.regNumber] = [];
      data.notifications[v.regNumber].unshift({ msg: `🤝 You are assigned as a volunteer for '${newEvent.title}' as '${v.role}'. ID: ${v.volunteerId}`, time: new Date().toISOString(), read: false });
    });
  }
  saveData(data);
...
app.get("/api/events", (req, res) => {
  ...
  const augmentedEvents = data.events.map(event => {
      ...
      return { ...event, creatorName: creator ? `${creator.name} ${creator.surname}` : 'Unknown Organizer', creatorGender: creator ? creator.gender : 'Other', media: eventMedia };
  });
...
app.get('/api/volunteers/:eventId', (req, res) => {
  ...
  const list = (event.volunteers || []).map(v => {
    const u = data.users.find(u => u.regNumber === v.regNumber);
    return { regNumber: v.regNumber, name: u ? `${u.name} ${u.surname || ''}`.trim() : v.name || v.regNumber, volunteerId: v.volunteerId };
  });
  res.json({ ok: true, volunteers: list });
});
...
app.post('/api/volunteers/add', (req, res) => {
  ...
  const volunteer = { regNumber, name: user.name, volunteerId: `V${String(nextIdx).padStart(2,'0')}`, role: roleName };
  event.volunteers.push(volunteer);
  if (!data.notifications) data.notifications = {};
  if (!data.notifications[regNumber]) data.notifications[regNumber] = [];
  data.notifications[regNumber].unshift({ msg: `🤝 You are assigned as a volunteer for '${event.title}' as '${roleName}'. ID: ${volunteer.volunteerId}`, time: new Date().toISOString(), read: false });
  saveData(data);
